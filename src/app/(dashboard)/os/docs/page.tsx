...
// Fetch system stats from JSON files
const fetchSystemStats = async (): Promise<SystemStats> => {
  try {
    const [opsResponse, skillsResponse] = await Promise.all([
      fetch('/data/ops-status.json', { cache: 'no-store' }),
      fetch('/data/skills.json', { cache: 'no-store' })
    ]);

    const opsData = opsResponse.ok ? await opsResponse.json() : null;
    const skillsData = skillsResponse.ok ? await skillsResponse.json() : null;

    return {
      agents: {
        total: opsData?.agents?.total || 0,
        active: opsData?.agents?.active || 0
      },
      crons: {
        total: opsData?.crons?.total || 0,
        active: opsData?.crons?.active || 0
      },
      skills: {
        total: skillsData?.skills?.length || 0
      },
      sessions: {
        active: opsData?.agents?.sessions?.length || 0
      }
    };
  } catch (error) {
    console.error('Error fetching system stats:', error);
    return { agents: { total: 0, active: 0 }, crons: { total: 0, active: 0 }, skills: { total: 0 }, sessions: { active: 0 } };
  }
};
...
- **Standups** are multi-agent conversations logged as transcripts`,
  ...
  {
    id: 'sys-stats', title: 'System Stats', slug: 'system-stats', category: 'system-stats', autoGenerated: true, updatedAt: new Date().toISOString(),
    content: `# System Stats

Live counts from the operational data system (auto-refreshes every 30s).

**This section displays real-time data from ops-status.json and skills.json files.**`,
  },
  {
