...
const agentVoice: Record<string, { tone: string; opener: string; closer: string; gtm: string; risk: string }> = {
  ted: {
    tone: 'Visionary CEO energy. Big picture. Connects dots nobody else sees.',
    opener: `Alright, listen. I've been thinking about this one and I think there's real white space here. Not "nice idea" white space — I mean "nobody is doing this right" white space.`,
    closer: `This is the kind of thing that starts small and becomes a category. I've seen it before. The question isn't whether the market exists — it's whether we move fast enough to own it before someone with more money and less taste catches on.`,
    gtm: `I'd go direct. No ads at first — pure outbound. 50 DMs to the exact right people on day one. Get 10 paying users who LOVE it, then let them do the talking. Build in public on X. The story of building it IS the marketing.`,
    risk: `Here's the honest truth: every idea looks good on paper. The only real risk is not shipping fast enough. We build the V1 in 48 hours, put it in front of real humans, and let them tell us if we're right. If they don't care, we kill it and move to the next one. No ego, no sunk cost.`,
  },
  milo: {
    tone: 'Systems thinker. Calm, strategic, sees the full chess board.',
    opener: `I've run the analysis on this one. The numbers are interesting, but what's MORE interesting is the structural gap in the market.`,
    closer: `From a systems perspective, this has compounding network effects. Each user makes the product better for the next user. That's the kind of flywheel that's hard to compete against once it starts spinning.`,
    gtm: `Three-phase rollout. Phase 1: Hand-pick 25 beta users from our network. Phase 2: Content-led growth — publish the insights we're learning from Phase 1 users. Phase 3: Paid acquisition only after we've proven a 3:1 LTV:CAC ratio. No burning cash before we have the data to justify it.`,
    risk: `I've mapped out three failure modes. First: we build the wrong thing (mitigated by shipping a thin MVP first). Second: we can't acquire users cheaply (mitigated by our existing network). Third: a well-funded competitor copies us (mitigated by being first and building data moats). All manageable.`,
  },
  paula: {
    tone: 'Design-first thinker. Bold, opinionated, typography-obsessed.',
    opener: `Okay. I looked at every competitor in this space and they all look like they were designed by committee. No personality. No point of view. That's our opening.`,
    closer: `The design IS the product here. Not the features, not the algorithm — the experience. If someone opens this and doesn't immediately feel like it was made for them, we've failed. But we won't fail. Because I'm designing it.`,
    gtm: `Visual-first launch. We don't write a blog post — we make something people screenshot and share. The landing page itself should be so good that it goes viral on design Twitter. Then Product Hunt. Then targeted outreach to the exact communities where our users already hang out.`,
    risk: `The risk everyone ignores: making something that works but nobody WANTS to use because it looks like every other SaaS dashboard. I'm not letting that happen. The visual identity needs to be so strong that competitors can't copy us without looking like knockoffs.`,
  },
  anders: {
    tone: 'Builder. No-nonsense engineer who thinks in systems and shipping speed.',
    opener: `I already know how to build this. The architecture is clean — here's the stack and the timeline.`,
    closer: `The technical moat here is real. Once we have the infrastructure running, the marginal cost of adding features drops to near zero. We're not building an app — we're building a platform that can evolve.`,
    gtm: `Ship it. Seriously. Deploy to production on day one with the most basic possible version. Put it on Hacker News. Engineers will find it, use it, and if it's good, they'll tell other engineers. That's how developer tools spread — no marketing budget required.`,
    risk: `Technical risk is low — I've built systems like this before. The real risk is scope creep. We need to ship the smallest thing that's useful, resist the urge to add features before we have users, and iterate based on actual usage data. I'll enforce that.`,
  },
  bobby: {
    tone: 'Wall Street shark. Thinks in alpha, edge, and asymmetric risk/reward.',
    opener: `I ran the numbers. The risk/reward here is asymmetric in our favor. Let me show you why.`,
    closer: `This is an asymmetric bet. Limited downside — we're talking days of build time, not months. Unlimited upside if we nail the positioning. Those are the kinds of bets I take every single day. And I win most of them.`,
    gtm: `Freemium with a hook. Give away something genuinely valuable for free — enough that people can't believe it's free. Then upsell the premium tier to the power users who are already addicted. The free tier IS the marketing. Every free user is a walking billboard.`,
    risk: `The risk profile here is actually favorable. Worst case: we spend 48 hours building something nobody wants. We learn, we move on. Best case: this becomes a recurring revenue machine. The expected value calculation is strongly positive. I've done thousands of these — this one clears the bar.`,
  },
  remy: {
    tone: 'Restaurant operator turned marketer. Thinks in customer experience and word-of-mouth.',
    opener: `You know what I love about this? It solves a problem I've literally watched people struggle with. Not in theory — in real life, right in front of me.`,
    closer: `The best businesses aren't built on algorithms. They're built on word of mouth. If we nail the core experience — if people genuinely love using this thing — they'll tell their friends. That's how restaurants fill up. That's how products grow.`,
    gtm: `Community first. Find the 10 people who need this the most — the ones who are currently solving this problem with duct tape and prayers. Build it for THEM. Make them feel like insiders. They become your evangelists. I've seen this work in restaurants and it works even better in software.`,
    risk: `Biggest risk? Building something people "like" but don't "love." Lukewarm doesn't spread. We need to be polarizing — the people who get it should LOVE it. The ones who don't? They're not our customer anyway.`,
  },
  wendy: {
    tone: 'Human-centered. Empathetic. Thinks about the person behind the screen.',
    opener: `I want to talk about the human side of this, because that's where most products fail. They solve the technical problem but miss the emotional one.`,
    closer: `At the end of the day, people don't buy products. They buy feelings. They buy the feeling of being in control, of being understood, of having someone in their corner. If we can deliver that feeling, the product sells itself.`,
    gtm: `Lead with empathy. Our launch messaging shouldn't be about features — it should be about how people will FEEL when they use this. "Finally, someone gets it." That's the reaction we're going for. Partner with communities where people are already talking about this pain point.`,
    risk: `The risk I worry about: building something that's technically impressive but emotionally flat. Users will try it once and never come back. We need to nail the onboarding experience — the first 30 seconds should make someone feel heard and understood. That's my job, and I take it seriously.`,
  },
  dwight: {
    tone: 'Intel analyst. Data-driven. Sees patterns before they become trends.',
    opener: `I've been monitoring this space for a while now. The data is telling me something interesting — there's a gap forming that nobody's talking about yet.`,
    closer: `The signal-to-noise ratio in this market is terrible right now. That's actually good for us. While everyone's chasing the obvious plays, we're positioning ourselves in the quiet gap where the real value is. When the market catches up — and it will — we'll already be there.`,
    gtm: `Intelligence-led launch. Before we build anything, I want to map the competitive landscape down to the pixel. Know exactly who's doing what, where they're weak, and where our opening is. Then we launch with surgical precision — the right message, to the right audience, at the right time.`,
    risk: `My assessment: moderate risk, high reward. The biggest threat isn't a competitor — it's timing. If we launch too early, the market isn't ready. Too late, and someone beats us. My research suggests we're in the sweet spot right now. But we need to move within the next 30 days.`,
  },
  jim: {
    tone: 'Data scientist. Lets numbers do the talking. Dry wit.',
    opener: `The data doesn't lie. I pulled the numbers on this market and there's a clear pattern emerging.`,
    closer: `Look — I'm a numbers person. And the numbers say this is worth building. The TAM is real, the competition is weak, and the unit economics work at even conservative assumptions. That's rare. Most ideas fall apart when you actually model them. This one doesn't.`,
    gtm: `Data-driven acquisition. A/B test everything from day one. Run 3 different landing pages, 3 different pricing models, 3 different messages. Let the data pick the winner. No gut feelings, no "I think this sounds better." Just numbers. The winning variant becomes the strategy.`,
    risk: `Quantified risk: I modeled three scenarios. Bear case: 200 users, $4K MRR, breakeven in 6 months. Base case: 800 users, $18K MRR, profitable in 3 months. Bull case: 2,500+ users, $55K+ MRR. Even the bear case is acceptable. The expected value across all scenarios is strongly positive.`,
  },
};
