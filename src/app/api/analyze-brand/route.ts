...
export async function POST(request: NextRequest) {
  try {
    ...
    const geminiResponse = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${process.env.GOOGLE_AI_KEY || (process.env.GOOGLE_AI_KEY || process.env.GOOGLE_API_KEY)}`,
      {
        method: 'POST',
